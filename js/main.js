...
function calcSum(memo = 0) {
    // В момент ВИКЛИКУ функції (це також відноситься до циклів, if-else, switch, try-catch, звичайних блоків {})
    // створюється ВЛАСНЕ оточення, яке містить силку на оточення в якому вона була створена (outer -> [[ Environment ]])
    ...
    // ВЛАСНЕ оточення що буде створено при визові функції:
    // Lexical Environment = {
    ...
    return (num) => {
        // ВЛАСНЕ оточення що створиться при ВИКЛИКУ функції:
        // Lexical Environment = {
        //   Environment Record: { num: ..., .... },
        //   outer: Env of calcSum
        // }
        ...
        // Враховуючи опис вище, @memo це змінна з зовні,
        // ми будемо мати доступ до неї при кожному
        // новому виклику функції. Вона як би замикається у
        // тілі цієї функції доки є хоч одна силка на цю функцію
        // В прикладі це змінна const sum
        return memo += num;
    };
}
